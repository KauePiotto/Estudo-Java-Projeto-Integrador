...
public class ExAlteracao {
	...
	private static void realizaOperacao() throws Exception {
		...
		try {
			...
			String nome = JOptionPane.showInputDialog("Digite o nome do aluno a ser alterado");
			ExConsultaNomeCompleto consulta = new ExConsultaNomeCompleto();
			alunos alterado = consulta.consultarNomeCompleto(nome);
			if (consulta != null) {
				int resp = JOptionPane.showConfirmDialog(null, "Confirma a alteração do aluno: \n" + alterado);
				if (resp == 0) {
					alterarAluno(alterado);
				} else {
					JOptionPane.showMessageDialog(null, "O aluno: " + alterado.getNome() + " NÃO foi alterado");
				}
			} else
				JOptionPane.showMessageDialog(null, "O aluno: " + alterado.getNome() + " NÃO foi encontrado");
	...
	public static alunos alterarAluno(alunos alterado) throws SQLException {
		ConectaMySQL cm = new ConectaMySQL();
		Connection con = cm.openDB();
		PreparedStatement pstm = con.prepareStatement(
				"UPDATE alunos SET " + " alunosNome=?, alunosTelefone=?, alunosNota=? WHERE idalunos=?");
		...
		pstm.setInt(4, (int) alterado.getId());
		alunos aposAlteracao = new alunos((int) alterado.getId(), nome, telefone, nota);
		pstm.execute();
