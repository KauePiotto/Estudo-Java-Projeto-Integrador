...
public class ExemploExclusao {
	...
	private static void realizaOperacao() throws Exception {
		...
		try {
			...
			alunos excluido = consulta.consultarNomeCompleto(nome);
			if (excluido == null) {
			...
			if (resp == 0) {
				st.executeUpdate("DELETE FROM Alunos WHERE alunosid='" + excluido.getId() + "'");
				JOptionPane.showMessageDialog(null,
						"O aluno: " + excluido.getNome() + " Foi excluído com sucesso!!!");
			} else
				JOptionPane.showMessageDialog(null, "O aluno: " + excluido.getNome() + " NÃO foi excluído");
